...
const HeroSection: React.FC<HeroSectionProps> = ({ images }) => {
  ...
  return (
    <header id="landing-hero" className="relative overflow-hidden text-white">
      ...
      <div className="max-w-6xl mx-auto px-6 py-16 md:py-24 relative z-10">
        <div className="max-w-3xl space-y-8 hero-text-shadow">
          <div className="hidden sm:inline-flex items-center gap-2 px-4 py-2 bg-white/10 rounded-full text-sm font-medium ring-1 ring-white/20 backdrop-blur hero-fade-1">
            <span className="material-symbols-outlined text-lg">apartment</span>
            Портал ТСЖ
          </div>
          <div className="space-y-3">
            <p className="text-sm uppercase tracking-[0.15em] text-white/80 font-semibold hero-fade-2">Новое Девяткино</p>
            <h1 className="text-4xl md:text-5xl font-bold leading-tight hero-fade-2">
              Поселок, который живет вместе с вами
            </h1>
            <p className="text-lg text-white/90 max-w-2xl hero-fade-3">
          ...
          <div className="flex flex-wrap items-center gap-4 hero-fade-4">
            <Link
              to="/dashboard"
              className="px-6 py-3 bg-accent text-white font-semibold rounded-xl shadow-sm hover:-translate-y-0.5 hover:bg-ink transition transform"
            >
              Перейти в кабинет
            </Link>
            <a
              href="#features"
              className="px-6 py-3 border border-white/60 text-white font-semibold rounded-xl hover:bg-white/10 transition"
            >
              Возможности
            </a>
          </div>
          <div className="flex flex-wrap gap-6 text-sm text-white/80 hero-fade-5">
